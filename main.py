...
def command_buat(args):
    if len(args) < 2:
        print("❌ Gunakan: buat file/folder <nama>")
        return
    tipe = args[0]
    nama = args[1]
    path = os.path.join(current_dir, nama)
    try:
        if tipe == "file":
            with open(path, "w") as f:
                f.write("")
            print(f"✅ File {nama} dibuat di {current_dir}")
        elif tipe == "folder":
            os.makedirs(path, exist_ok=True)
            print(f"✅ Folder {nama} dibuat di {current_dir}")
        else:
            print("❌ Tipe harus 'file' atau 'folder'")
    except Exception as e:
        print(f"❌ Gagal membuat {tipe}: {e}")

def command_hapus(args):
    if len(args) == 0:
        print("❌ Masukkan nama file/folder yang akan dihapus")
        return
    ...
    try:
        if os.path.isdir(path):
            import shutil
            shutil.rmtree(path)
            print(f"✅ Folder {args[0]} dihapus")
        elif os.path.isfile(path):
            os.remove(path)
            print(f"✅ File {args[0]} dihapus")
        else:
